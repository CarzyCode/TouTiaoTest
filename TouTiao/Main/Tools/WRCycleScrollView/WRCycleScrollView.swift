...
/// pageControl相关
    ...
    var currentDotColor: UIColor = UIColor.blue {
        didSet {
            self.pageControl?.currentPageIndicatorTintColor = currentDotColor
        }
    }
    var otherDotColor: UIColor = UIColor.white {
        didSet {
...
extension WRCycleScrollView
{
    ...
    {
        let pageW = 5 * (descTextArray?.count)!
        let pageH:CGFloat = 20
        let pageX = bounds.width - CGFloat(pageW) - 40
        let pageY = bounds.height -  pageH
        self.pageControl?.frame = CGRect(x:pageX, y:pageY, width:CGFloat(pageW), height:pageH)
    }
}
